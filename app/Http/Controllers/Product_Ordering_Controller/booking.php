    ...
    public function  order_proceed(Request $request)
    {
        ...
        if (session('cart')) {
            ...
            foreach (session('cart') as $id => $details) {
                ...
                $order_details = strlen($order_details) > 0 ? $order_details . '<br>' : $order_details;


                $order_details = $order_details .
                    ('Product Name:' . $details["item_name"] . '<br> Quantity: ' . $details["item_quantity"] .'<br> Price:' . $details["Final_Price"]);
        ...
        if ($p_method == 'Online') {
            ...
            return redirect()->back()->with('status', 'payment method online not working!');
        } else {
